...
async function postChatMessage(chatId, text) {
  ...
  // Firefox data consent: sending chat messages is optional and can be revoked in about:addons.
  const allowed = await browser.permissions.contains({ data_collection: ['personalCommunications'] });
  if (!allowed) throw new Error('Firefox permission to send Teams messages is not granted. Click "Allow Teams access" in settings');
